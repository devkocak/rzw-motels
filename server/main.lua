		...
		for doorindex,_ in pairs(v.doors) do
			...
			if motels[v.motel].rooms[doorindex].players and GetResourceState('ox_inventory') == 'started' then
				for id,_ in pairs(motels[v.motel].rooms[doorindex].players) do
					...
					exports.ox_inventory:RegisterStash('stash_yume_rusun_'..stashid, 'Storage', 100, 2500000, false)
					exports.ox_inventory:RegisterStash('fridge_yume_rusun_'..stashid, 'Fridge', 100, 500000, false)
				end
...
lib.callback.register('wl-motels:rentaroom', function(src,data)
	...
	if not motels[data.motel].rooms[data.index].players[identifier] and data.duration > 0 then
		...
		motels[data.motel].rooms[data.index].players[identifier].duration = (getTanggal1() + ( data.duration * rental_period[data.rental_period]))
...
lib.callback.register('wl-motels:payrent', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('wl-motels:getMotels', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	return motels, os.time()
end)

lib.callback.register('wl-motels:motelkey', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local metadata = {
		type = data.motel,
		serial = data.index,
...
lib.callback.register('wl-motels:buymotel', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	local money = xPlayer.getAccount(data.payment).money
...
lib.callback.register('wl-motels:removeoccupant', function(src,data,index,player)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('wl-motels:addoccupant', function(src,data,index,player)
	local xPlayer = GetPlayerFromId(src)
...
lib.callback.register('wl-motels:editrate', function(src,motel,rate)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('wl-motels:addemployee', function(src,motel,id)
	local xPlayer = GetPlayerFromId(src)
...
lib.callback.register('wl-motels:removeemployee', function(src,motel,identifier)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	if motels[motel].owned == xPlayer.identifier then
...
lib.callback.register('wl-motels:transfermotel', function(src,motel,id)
	local xPlayer = GetPlayerFromId(src)
	local toPlayer = GetPlayerFromId(tonumber(id))
	local motels = GlobalState.Motels
	if motels[motel].owned == xPlayer.identifier and toPlayer then
...
lib.callback.register('wl-motels:sellmotel', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
lib.callback.register('wl-motels:withdrawfund', function(src,motel,amount)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
	if motels[motel].owned == xPlayer.identifier then
...
local invoices = {}
lib.callback.register('wl-motels:sendinvoice', function(src,motel,data)
	...
	if motels[motel].owned == xPlayer.identifier or motels[motel].employees[xPlayer.identifier] then
		...
		invoices[id] = data[2]
		TriggerClientEvent('wl-motels:invoice',toPlayer,{
			motel = motel,
			amount = data[2],
			description = data[3],
			id = id,
...
lib.callback.register('wl-motels:payinvoice', function(src,data)
	local xPlayer = GetPlayerFromId(src)
	local motels = GlobalState.Motels
...
local routings = {}
lib.callback.register('wl-motels:SetRouting', function(src,data,Type)
	local xPlayer = GetPlayerFromId(src)
	if Type == 'enter' then
...
lib.callback.register('wl-motels:MessageOwner', function(src,data)
	local motels = GlobalState.Motels
	...
	if xPlayer then
		TriggerClientEvent('wl-motels:MessageOwner',xPlayer.source, data)
		return true
	end
	return false
end)

RegisterServerEvent("wl-motels:Door")
AddEventHandler('wl-motels:Door', function(data)
	local source = source
	TriggerClientEvent('wl-motels:Door', -1, data)
...
lib.callback.register('wl-motels:checkMemilikiRusun', function(source, data)
	local dataMotels = MySQL.query.await('SELECT * FROM yume_motels')
	return dataMotels
end)