...
local db = setmetatable({},{
	__call = function(self)
		...
		self.insert = function(...)
			...
			return MySQL.insert.await(str:format('yume_motels','motel','hour_rate','revenue','employees','rooms','owned'),{...})
		end

		self.update = function(column, where, string, data)
			local str = 'UPDATE %s SET %s WHERE %s = ?'
			return MySQL.update(str:format('yume_motels',column,where),{data,string})
		end

		self.updateall = function(pattern, where, string, ...)
			local str = 'UPDATE %s SET % WHERE %s = ?'
			local str = 'UPDATE yume_motels SET '..pattern..' WHERE '..where..' = ?'
			local data = {...}
		...
		self.query = function(column, where, string)
			...
			return MySQL.query.await(str:format(column,'yume_motels',where),{string})
		end

		self.fetchAll = function()
			local str = 'SELECT * FROM yume_motels'
...
Citizen.CreateThreadNow(function()
	local success, result = pcall(MySQL.scalar.await, 'SELECT 1 FROM yume_motels')
	if not success then
		MySQL.query.await([[CREATE TABLE `yume_motels` (
	...
	for k,v in pairs(config.motels) do
		local query = MySQL.query.await('SELECT rooms FROM yume_motels WHERE `motel` = ?',{v.motel})
		if not query[1] then
			local doors = {}
