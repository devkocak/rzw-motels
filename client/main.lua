...
RegisterNetEvent('wl-motels:invoice')
AddEventHandler('wl-motels:invoice', function(data)
	local motels = GlobalState.Motels
    local buy = lib.alertDialog({
		header = 'Invoice',
		content = '![motel](nui://wl-motels/data/image/'..data.motel..'.png) \n ## INFO \n **Description:** '..data.description..'  \n  **Amount:** $ '..data.amount..'  \n **Payment method:** '..data.payment,
		centered = true,
		labels = {
			cancel = 'close',
			confirm = 'Pay'
	...
	if buy ~= 'cancel' then
		local success = lib.callback.await('wl-motels:payinvoice',false,data)
		if success then
...
RegisterNetEvent('wl-motels:Door', function(data)
...
Door = function(data)
    ...
    if moteldoor and DoesPlayerHaveAccess(motel.rooms[data.index].players)
        ...
        TriggerServerEvent('wl-motels:Door', {
            motel = data.motel,
            index = data.index,
            coord = data.coord,
			Mlo = data.Mlo,
		...
		if data.Mlo then
			local doorindex = data.index + (joaat(data.motel))
			text = DoorSystemGetDoorState(doorindex) == 0 and 'Mengunci Pintu Rusun' or 'Membuka Pintu Rusun'
		else
			text = not moteldoor?.lock and 'Mengunci Pintu Rusun' or 'Membuka Pintu Rusun'
		end
		Wait(1000)
		...
		Notify('Kamu Tidak Memiliki Akses Kamar Ini', 'error')
    end
end
...
LockPick = function(data)
	...
	if success then
		...
		local doorindex = data.index + (joaat(data.motel))
		Notify(DoorSystemGetDoorState(doorindex) == 0 and 'Mengunci Pintu Rusun' or 'Membuka Pintu Rusun', 'inform')
	end
end
...
MyRoomMenu = function(data)
	...
	local options = {
		{
			title = 'Kamar Nomor : '..data.index,
			icon = 'fas fa-door-closed',
		},
		{
			title = 'Jatuh Tempo : '..data.duration,
			icon = 'fas fa-clock',
		},
		-- {
		...
		-- 	icon = 'key',
		-- 	onSelect = function()
		-- 		local success = lib.callback.await('wl-motels:motelkey',false,{
		-- 			index = data.index,
		-- 			motel = data.motel,
		-- 		})
		-- 		if success then
		...
		{
			title = 'Berhenti',
			description = 'Memberhentikan Sewa Rusun',
			icon = 'ban',
			onSelect = function()
				if isRentExpired(data) then
				...
				local End = lib.alertDialog({
					header = '## Warning',
					content = 'Kamu tidak lagi memiliki akses ke pintu dan brankas Anda.',
					centered = true,
					labels = {
						cancel = 'Batal',
						confirm = 'Berhenti',
				...
				local success = lib.callback.await('wl-motels:removeoccupant',false,data,data.index,PlayerData.identifier)
				if success then
					Notify('Berhasil Berhenti Menyewa Rusun '..data.index,'success')
				else
...
function checkKamar(data)
	local motels , time = lib.callback.await('wl-motels:getMotels',false)
	local rate = motels[data.motel].hour_rate or data.rate
	local kamarSaya = {}
	for doorindex,v in ipairs(data.doors) do
		...
		if motels[data.motel].rooms[doorindex].players[PlayerData.identifier] then
			table.insert(kamarSaya, {
				index = doorindex,
			})
			break
		end
...
function sudahPunya(data, index)
	local motels , time = lib.callback.await('wl-motels:getMotels',false)
	local rate = motels[data.motel].hour_rate or data.rate
	local playerroom = motels[data.motel].rooms[index].players[PlayerData.identifier]
	local duration = playerroom?.duration
	local occupants = CountOccupants(motels[data.motel].rooms[index].players)
	local options = {}
	local duration_left = lib.callback.await('wl-tools:server:formatTanggal', false, duration)
	table.insert(options, {
		title = 'Options Rusunku #'..index,
		description = 'Melihat Jatuh Tempo, Memberhentikan Sewa Rusun',
		icon = 'cog',
		onSelect = function()
			return MyRoomMenu({
	...
	lib.showContext('rentmenuSudahAda')
end
...
RoomList = function(data)
	...
	if #kamarSaya >= 1 then
		local playerroom = motels[data.motel].rooms[kamarSaya[1].index].players[PlayerData.identifier]
		local duration = playerroom?.duration
		local occupants = CountOccupants(motels[data.motel].rooms[kamarSaya[1].index].players)
		local duration_left = lib.callback.await('wl-tools:server:formatTanggal', false, duration)
		table.insert(options,{
			title = 'My Room Door #'..kamarSaya[1].index..' Options',
			description = 'Pay your rent or request a motel key',
			icon = 'cog',
			onSelect = function()
				return MyRoomMenu({
					payment = data.payment,
					index = kamarSaya[1].index,
					motel = data.motel,
					duration = duration_left,
					rate = rate,
					rental_period = data.rental_period
		...
		for doorindex,v in ipairs(data.doors) do
			if v['door'] then
				...
				if occupants < data.maxoccupants and not duration then
					table.insert(options,{
						title = 'Menyewa Kamar #'..doorindex,
						description = 'Nomor Kamar #'..doorindex..' \n Max Penghuni : '..occupants..'/'..data.maxoccupants,
						icon = 'door-closed',
						onSelect = function()
							local alert = lib.alertDialog({
								header = 'Sewa Rusun',
								content = 'Apakah Anda Yakin Ingin Menyewa Rusun Dengan Nomor Kamar #'..doorindex,
								centered = true,
								cancel = true,
								labels = {
									cancel = 'Batal',
							...
							local success = lib.callback.await('wl-motels:rentaroom',false,{
								index = doorindex,
								motel = data.motel,
								duration = 30,
								rate = rate,
							...
							if success then
								Notify('Berhasil Menyewa Kamar', 'success')
							else
...
local function checkJobRusun(data, job)
	local found = false
	for _, role in ipairs(data.job) do
		if role == job then
			found = true
			break  -- Keluar dari loop jika inputan ditemukan
		end
	end
	if found then
		return true
...
local function checkMemilikiRusun(xPlayer)
	local getData = lib.callback.await('wl-motels:checkMemilikiRusun', false)
	local found = false
	local nameMotel
	for ab, cd in ipairs(getData) do
		local getRooms = json.decode(cd.rooms)
		for ef,gh in pairs(getRooms) do
			for hi,jk in pairs(gh.players) do
				if hi == xPlayer.identifier then
					found = true
					nameMotel = cd.motel
					break
				end
	...
	if found then
		return true, nameMotel
	else
		return false, nameMotel
	end
end

lib.callback.register('wl-motel:client:getSpawn', function(xPlayer)
    local getData = lib.callback.await('wl-motels:checkMemilikiRusun', false)
	local found = false
	local data, getCoods
	for ab, cd in ipairs(getData) do
		...
		for ef,gh in pairs(getRooms) do
			for hi,jk in pairs(gh.players) do
				if hi == xPlayer.identifier then
					for lm, nm in pairs(config.motels) do
						if cd.motel == nm.motel then
							getCoods = nm.doors[ef]
							break
						end
					end
					found = true
...
MotelRentalMenu = function(data)
	local motels = GlobalState.Motels
	local rate = motels[data.motel].hour_rate or data.rate
	local xPlayer = lib.callback.await('wl-tools:server:getDataUsers:xPlayer', false)
	local options = {}
	local checkKamar = checkKamar(data)
	local checkMemilikiRusun, nameMotel = checkMemilikiRusun(xPlayer)
	if not checkMemilikiRusun or nameMotel == data.motel then
		if data.job then
			local checkJob = checkJobRusun(data, string.gsub(xPlayer.job.name, "off", ""))
			if not checkJob then return TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Kamu Tidak Dapat Mengakses Rusun Ini' }) end
		end
		if not data.manual then
			if #checkKamar >= 1 then
				return sudahPunya(data, checkKamar[1].index)
			else 
				table.insert(options,{
					title = 'Menyewa Rusun',
					description = 'Siapa Cepat Dia Dapat, Sewa Rusun Bulanan Dengan Harga '..lib.callback.await('wl-tools:server:formatMataUang:ym',false, rate*30)..'/Bulan',
					icon = 'hotel',
					onSelect = function()
		...
		if data.manual then
			Notify('Rusun ini secara manual menerima penghuninya  \n  Hubungi Pemilik')
			Wait(1500)
		...
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Kamu Sudah Memiliki Rusun' })
	end
end

SendMessageApi = function(motel)
	local message = lib.alertDialog({
		header = 'Apakah Anda ingin Mengirim Pesan kepada Pemiliknya?',
		content = '## Kirim Pesan Ke Pemilik Rusun',
		centered = true,
		labels = {
			cancel = 'Batalkan',
			confirm = 'Mengirim',
	...
	local input = lib.inputDialog('Mengirim Pesan', {
		{type = 'input', label = 'Nama', description = 'Masukan Nama Lengkap Anda', icon = 'fas fa-user-tie', required = true},
		{type = 'textarea', label = 'Pesan', description = 'Masukan Isi Pesan Yang Ingin Disampaikan Ke Pemilik', icon = 'fas fa-envelope-open-text', required = true},
		{type = 'number', label = 'Nomot HP', icon = 'phone', required = true},
	})
	if input == nil or input[1] == nil or input[2] == nil or input[3] == nil then return end
...
Owner.Rooms.Occupants = function(data,index)
	...
	for player,char in pairs(players) do
		...
		table.insert(options,{
			title = 'Occupant '..name,
			description = 'Jatuh Tempo : '..duration_left,
			icon = 'hotel',
			onSelect = function()
				...
				local success = lib.callback.await('wl-motels:removeoccupant',false,data,index,player)
				if success then
		...
		for i = 1, data.maxoccupants-#options do
			table.insert(options,{
				...
				onSelect = function()
					...
					local success = lib.callback.await('wl-motels:addoccupant',false,data,index,input)
	...
	local options = {
		{
			...
			onSelect = function()
				...
				if not input then return end
				local success = lib.callback.await('wl-motels:addemployee',false,data.motel,input[1])
				if success then
    ...
    if motel and motel.employees then
       for identifier,name in pairs(motel.employees) do
          table.insert(options,{
			...
			icon = 'hotel',
			onSelect = function()
				local success = lib.callback.await('wl-motels:removeemployee',false,data.motel,identifier)
					if success then
...
MotelOwner = function(data)
	...
	if not motels[data.motel].owned then
		...
		if buy ~= 'cancel' then
			local success = lib.callback.await('wl-motels:buymotel',false,data)
			if success then
		...
		local options = {
			...
			},
			{
				...
				onSelect = function()
					...
					local success = lib.callback.await('wl-motels:sendinvoice',false,data.motel,input)
					if success then
		...
		if motels[data.motel].owned == PlayerData.identifier then
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('wl-motels:editrate',false,data.motel,input[1])
					if success then
			...
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('wl-motels:withdrawfund',false,data.motel,input[1])
					if success then
			...
			table.insert(options,{
				...
				onSelect = function()
					...
					if not input then return end
					local success = lib.callback.await('wl-motels:transfermotel',false,data.motel,input[1])
					if success then
			...
			table.insert(options,{
				...
				onSelect = function()
					...
					if sell ~= 'cancel' then
						local success = lib.callback.await('wl-motels:sellmotel',false,data)
						if success then
...
function Teleport(x, y, z, h ,exit)
	...
	if exit then
		...
		DeleteEntity(house)
		lib.callback.await('wl-motels:SetRouting',false,data,'exit')
		shelzones = {}
...
EnterShell = function(data,login)
	...
	if motels[data.motel].rooms[data.index].lock and not login then
		Notify('Pintu Rusun Terkunci', 'error')
		return false
	end
	...
	lib.callback.await('wl-motels:SetRouting',false,data,'enter')
	inhouse = true
	Wait(1000)
				...
				for k,v in pairs(data.doors) do
					if #(vec3(bullet.x,bullet.y,bullet.z) - vec3(v.door.x,v.door.y,v.door.z)) < 2 and motels[data.motel].rooms[k].lock then
						...
						if data.Mlo then
							...
							text = DoorSystemGetDoorState(doorindex) == 0 and 'Kamu Menghancurkan Pintu Rusun'
						else
							text = 'Kamu Menghancurkan Pintu Rusun'
						end
						Notify(text,'warning')
					end
				end
...
RegisterNetEvent('wl-motels:MessageOwner', function(data)
